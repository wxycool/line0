...
myCtrl.controller("SearchTopCtrl", [ "$scope", "$rootScope", "$state", "$stateParams", function(a, b, c, d) {
    ...
    function g() {
        ...
        var a = "http://handset.line0.com/ws/handset/v3/mcms/bannerList?jsonp=getBannerList&clientFlag=line0123456789abcdef987opqxyzline0&cityId=" + cityId + "&areaId=null&userX=" + longitude + "&userY=" + latitude;
        c.jsonp(a).success();
    }
    ...
    }), cookie.get("firestLogin") || (a.firstLogin = !0, cookie.set("firestLogin", 1, 262800)), 
    getLocation(g), d.getBannerList = function(b) {
        //console.log(data);
        a.bannerList = b.response.bannerList, //console.log($scope.bannerList);
        a.banner();
    }, /* 轮播图 */
    a.banner = function() {
    ...
    //初始化页面数据——定义
    function e() {
        var a = "http://handset.line0.com/ws/handset/v3/shop/shopList?jsonp=getShopList&cityId=" + cityId + "&userX=" + longitude + "&userY=" + latitude + "&toPage=1&pageRows=20&shopType=" + b.cid + "&distance=" + b.distance + "&sort=" + b.sort;
        //console.log(url);
        c.jsonp(a).success();
    }
    //加载更多数据-----定义
    function f() {
        ...
        var d = "http://handset.line0.com/ws/handset/v3/shop/shopList?jsonp=getMoreShopByPage&cityId=" + cityId + "&userX=" + longitude + "&userY=" + latitude + "&toPage=" + a.curPage + "&pageRows=20&shopType=" + b.cid + "&distance=" + b.distance + "&sort=" + b.sort;
        c.jsonp(d).success();
    }
    //初始化数据
    ...
    d.getStore = function(e) {
        ...
        if ("0000" !== a.cate[i].typeId) {
            var k = "http://handset.line0.com/ws/handset/v3/shop/productList?jsonp=getStoreCate&clientFlag=line0123456789abcdef987opqxyzline0&shopId=" + b.storeId + "&shopProdTypeId=" + a.cate[i].typeId;
            //console.log(url);
            c.jsonp(k).success();
        }
...
myCtrl.controller("StoredCtrl", [ "$scope", "$rootScope", "$http", "$window", "$stateParams", function(a) {
    ...
    function f() {
        ...
        var a = "http://handset.line0.com/ws/handset/v3/shop/productdetail?jsonp=getGoods&clientFlag=line0123456789abcdef987opqxyzline0&cityId=" + cityId + "&userX=" + longitude + "&userY=" + latitude + "&productId=" + e.gid;
        //console.log(url);
        c.jsonp(a).success();
    }
    // 初始化数据
    ...
    a.GoodsCommentCtrl = function() {
        if (0 == h) {
            var b = "http://handset.line0.com/ws/handset/v3/shop/productappraises?jsonp=getGoodsComment&pageNum=1&pageRow=200&productId=" + e.gid;
            //console.log(url);
    ...
    a.GoodsArticleCtrl = function() {
        if (0 == i) {
            var b = "http://handset.line0.com/ws/handset/v3/shop/productdetailpicture?jsonp=getGoodsHTML";
            //console.log(url);
            c.jsonp(b, {
                params: {
                    productId: e.gid
...
myCtrl.controller("PromotionCtrl", [ "$scope", function(a) {
    ...
    function e() {
        ...
        var c = "http://handset.line0.com/ws/handset/v3/mcms/promotion?jsonp=getMorePromotion", d = {
            params: {
                userX: longitude,
                userY: latitude,
                cityId: cityId,
    ...
    a.StatePromotionMain = !1, a.StatePromotionRefresh = !0, getLocation(d), c.getPromotion = function(b) {
        //console.log(data.response);
        if (0 == b.response.size) d(); else {
            imgLen = 0;
            for (var c = 0; c < b.response.shopList.length; c++) imgLen += b.response.shopList[c].productList.length > 3 ? 3 : b.response.shopList[c].productList.length;
            a.promotions = b.response.shopList, a.curPage = b.response.page.curPage, a.totalPage = b.response.page.pageAmount, 
            //console.log($scope.promotions);
            a.StatePromotionMain = !0, a.StatePromotionRefresh = !1;
        }
    }, $.getScript("lib/other/iscroll-probe.min.js", function() {
        myScrollOne = new IScroll("#scroller", {
            tap: !0,
            preventDefault: !1,
            probeType: 2
        }), document.addEventListener("touchmove", function(a) {
    ...
    }), c.getMorePromotion = function(b) {
        //console.log(data.response);
        if (0 == b.response.size) e(); else {
            imgLen = 0;
            for (var c = 0; c < b.response.shopList.length; c++) imgLen += b.response.shopList[c].productList.length > 3 ? 3 : b.response.shopList[c].productList.length;
            //console.log(imgLen);
            a.promotions = a.promotions.concat(b.response.shopList);
        }
    }, /* 监听完成传播事件 */
    a.$on("ngRepeatFinished", function() {
        //console.log('start');
        $.getScript("lib/other/iscroll-probe.min.js", function() {
            $(".spin_refresh").hide(), g.className = "", g.innerHTML = "下拉准备刷新...", f = 0, myScrollOne.refresh(), 
            // 加载更多数据----绑定滚动事件
            myScrollOne.on("scroll", function() {
                //console.log(this.y, (this.maxScrollY - 5));
                0 != f || g.className.match("show|loading") || this.y > 5 || this.y < this.maxScrollY - 5 && (//下拉准备刷新效果
                //console.log('down');
                g.className = "show", myScrollOne.refresh(), f = 1);
    ...
    a.StateHotMain = !0, a.StateHotRefresh = !0, $.getScript("lib/other/iscroll-probe.min.js", function() {
        ...
        imgLen = b.response.guessLikeOnes.length;
    }, a.$on("ngRepeatFinished", function() {
        //$('img').load(function () {
        //console.log('start');
        ...
        imgLen = b.response.guessLikeOnes.length, //console.log(data.response);
    ...
    return {
        ...
        link: function(a, b, c) {
            ...
            loadImage(d, function() {
                imgIndex++, //console.log(imgIndex, imgLen,  scope.$index);
                imgIndex >= imgLen && (imgIndex = 0, //console.log('onFinishRenderFiltersEnd');
                a.$emit("ngRepeatFinished"));
            });
        }
    };
}), myDirective.directive("wxyOnFinishRenderPromotion", function() {
    return {
        restrict: "AE",
        link: function(a, b, c) {
            var d = c.ngSrc;
...
myFilter.filter("fixWxURL", function() {
    ...
    return function(a) {
        return void 0 != a && -1 == a ? "d_n" : void 0;
    };
}), myFilter.filter("wxyToImgSrcNoimage", function() {
    return function(a) {
        return console.log(a), "http://img.line0.com/static/noimage.jpg" == a ? a : a + "_200x.jpg";
    };
});
